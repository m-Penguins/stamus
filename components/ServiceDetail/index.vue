...
const { data: serviceData } = await useFetch(
  `${apiBaseUrl}services/${serviceId}`,
  {
    query: {
      populate:
        "photoBanner.*,blocks.*, blocks.price_lists.servicePrice.Sale_popular.*, blocks.specialists.fotoSpecialist.*, blocks.specialists.achievements.icon.*, blocks.reviews.*, blocks.image.*, blocks.video.videoPreview.*, blocks.banner.image.*, blocks.item.*, blocks.articles.fotoArticles.*",
    },
  },
);
